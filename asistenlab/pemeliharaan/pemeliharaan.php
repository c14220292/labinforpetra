...
// First, get all lab codes assigned to this assistant
$lab_codes = array();
$sql_labs = "SELECT kode_lab FROM pengguna_lab WHERE id_pengguna = ?";
$stmt = $conn->prepare($sql_labs);
$stmt->bind_param("i", $id_pengguna);
$stmt->execute();
$result_labs = $stmt->get_result();

while ($row = $result_labs->fetch_assoc()) {
    $lab_codes[] = $row['kode_lab'];
}
...
// Base query
$sql = "SELECT p.id_pemeliharaan, p.id_perlengkapan, p.laboratorium, p.waktu_pemeliharaan, 
                p.waktu_pengembalian, p.deskripsi_pemeliharaan, p.status, 
                pl.nama_perlengkapan, pl.set_komputer 
        FROM pemeliharaan p 
        JOIN perlengkapan pl ON p.id_perlengkapan = pl.id_perlengkapan";

// Add conditions based on lab access
if (!empty($lab_codes)) {
    $placeholders = implode(',', array_fill(0, count($lab_codes), '?'));
    $sql .= " WHERE p.laboratorium IN ($placeholders)";

    if (!empty($search)) {
        $sql .= " AND (pl.nama_perlengkapan LIKE '%$search_escaped%' 
                OR pl.set_komputer LIKE '%$search_escaped%' 
                OR pl.kode_seri LIKE '%$search_escaped%')";
    }
} else {
    // If assistant has no lab access, show empty result
    $sql .= " WHERE 1=0";
}
...
if (!empty($lab_codes)) {
    $types = str_repeat('s', count($lab_codes));
    $stmt->bind_param($types, ...$lab_codes);
}
$stmt->execute();
...
<body>
    ...
    <main>
        ...
        <?php if (empty($lab_codes)): ?>
            <div style="color: red; margin-bottom: 20px;">
                Anda tidak memiliki akses ke laboratorium manapun.
            </div>
        <?php else: ?>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" style="text-align:center;">Tidak ada data pemeliharaan untuk laboratorium Anda.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="modal-content" style="max-width: 800px;">
            ...
            <form id="finishForm" method="POST" action="finish_pemeliharaan.php">
                ...
                <div class="dropdown-container">
                    <div class="dropdown-group">
                        ...
                        <select id="labSelect" name="laboratorium" onchange="filterPemeliharaan()">
                            ...
                            <?php
                            // Only show labs that the assistant has access to
                            if (!empty($lab_codes)) {
                                $placeholders = implode(',', array_fill(0, count($lab_codes), '?'));
                                $labQuery = "SELECT DISTINCT laboratorium FROM pemeliharaan WHERE laboratorium IN ($placeholders)";
                    ...
                    <div class="dropdown-group">
                        ...
                        <select id="setSelect" name="set_komputer" onchange="filterPemeliharaan()">
                            ...
                            <?php
                            // Only show sets from labs the assistant has access to
                            if (!empty($lab_codes)) {
                                $placeholders = implode(',', array_fill(0, count($lab_codes), '?'));
                                $setQuery = "SELECT DISTINCT pl.set_komputer 
                                            FROM perlengkapan pl
                                            JOIN pemeliharaan p ON pl.id_perlengkapan = p.id_perlengkapan
                                            WHERE p.laboratorium IN ($placeholders)";
                ...
                <table>
                    ...
                    <tbody>
                        <?php
                        // Only show maintenance records from labs the assistant has access to
                        if (!empty($lab_codes)) {
                            $placeholders = implode(',', array_fill(0, count($lab_codes), '?'));
                            $filterQuery = "SELECT p.id_pemeliharaan, pl.nama_perlengkapan, p.laboratorium, p.waktu_pemeliharaan, pl.set_komputer, pl.kode_seri 
                                            FROM pemeliharaan p 
                                            JOIN perlengkapan pl ON p.id_perlengkapan = pl.id_perlengkapan 
                                            WHERE p.status = 'Dalam Proses'
                                            AND p.laboratorium IN ($placeholders)";
