...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-2xl font-bold text-petra-blue border-b-4 border-petra-orange pb-2 mb-6">Tambah Pengguna</h2>
        ...
        <form action="{{ route('admin.pengguna.store') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan pengguna ini?')">
            @csrf
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="nama_pengguna" value="{{ old('nama_pengguna') }}" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-petra-orange" required>
                </div>
                ...
                <div>
                    ...
                    <select name="role" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-petra-orange" required>
                        <option value="">-- Pilih Role --</option>
                        <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Admin</option>
                        <option value="kepalalab" {{ old('role') == 'kepalalab' ? 'selected' : '' }}>Kepala Lab</option>
                        <option value="asistenlab" {{ old('role') == 'asistenlab' ? 'selected' : '' }}>Asisten Lab</option>
                    </select>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <label class="flex items-center space-x-2 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 
                            ...
                            <input type="checkbox" name="lab_akses[]" value="{{ $lab->kode_lab }}" 
                                   {{ in_array($lab->kode_lab, old('lab_akses', [])) ? 'checked' : '' }}
                                   class="rounded">
