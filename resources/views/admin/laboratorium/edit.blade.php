...
<div class="max-w-md mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('admin.laboratorium.update', $laboratorium) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menambahkan laboratorium ini?')">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="gedung" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-petra-orange" required>
                        <option value="P" {{ old('gedung', $laboratorium->gedung) == 'P' ? 'selected' : '' }}>P</option>
                        <option value="T" {{ old('gedung', $laboratorium->gedung) == 'T' ? 'selected' : '' }}>T</option>
                    </select>
                </div>
