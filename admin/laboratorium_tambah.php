...
// Proses penambahan laboratorium
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $kode_lab = $_POST['kode_lab'];
    $nama_lab = $_POST['nama_lab'];
    $gedung = $_POST['gedung'];

    // Query untuk menambahkan laboratorium
    $sql = "INSERT INTO laboratorium (kode_lab, nama_lab, gedung) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sss", $kode_lab, $nama_lab, $gedung);

    if ($stmt->execute()) {
        header("Location: laboratorium.php?message=Laboratorium berhasil ditambahkan");
        exit();
    } else {
...
<body>
    ...
    <main>
        ...
        <form method="POST" action="" onsubmit="return confirm('Apakah Anda yakin ingin menambahkan laboratorium ini?');">
            ...
            <input type="text" id="kode_lab" name="kode_lab" required>
