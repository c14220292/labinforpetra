<?php
session_start();
// Pastikan pengguna sudah login dan admin
...
if (!$user) {
    header("Location: pengguna.php?error=Pengguna tidak ditemukan");
    exit();
}
...
// Ambil lab yang sudah diakses oleh pengguna dari tabel pengguna_lab
$akses_stmt = $conn->prepare("SELECT kode_lab FROM pengguna_lab WHERE id_pengguna = ?");
$akses_stmt->bind_param("i", $id_pengguna);
$akses_stmt->execute();
$akses_result = $akses_stmt->get_result();

$lab_akses = [];
while ($row = $akses_result->fetch_assoc()) {
    $lab_akses[] = $row['kode_lab'];
}
?>
...
<body>
    ...
    <div class="menu-section">
        ...
        <form action="update_pengguna.php" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengubah data pengguna ini?');">
            ...
            <select name="role" required>
                ...
                <option value="kepalalab" <?= $user['role'] == 'kepalalab' ? 'selected' : '' ?>>Kepala Lab</option>
                <option value="asistenlab" <?= $user['role'] == 'asistenlab' ? 'selected' : '' ?>>Asisten Lab</option>
            </select>

            <label>Akses Laboratorium:</label>
